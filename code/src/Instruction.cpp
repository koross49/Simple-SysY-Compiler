...
//如果 %condition 为真，则跳转到 %trueBlock，否则跳转到 %falseBlock
...
//内存分配
//%allocatedMemory = alloca i32, align 4
...
{
    operands.push_back(dst);//保存返回值的寄存器
    dst->setDef(this);
    for(auto param : params){//保存参数的寄存器
...
// %result = call i32 @addTwoNumbers(i32 5, i32 7)
...
//store i32 42, i32 %myVariable, align 4
...
void IntFloatCastInstructionn::output() const
{
    ...
    switch(opcode) {
        case I2F:
            castType = "sitofp";//有符号整数转为浮点数
            break;
        case F2I:
            castType = "fptosi";//浮点转为有符号整数
            break;
        default:
            castType = "";
            break;
